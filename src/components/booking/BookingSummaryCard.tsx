...
export function BookingSummaryCard({ hotel, room, checkIn, checkOut, nights, adults, children, subtotal, taxAmount, discount, total }: Props) {
  return (
    <div className="rounded-lg border border-neutral-200 p-4">
      <div className="flex gap-3">
        <img src={hotel.images[0]?.url} alt={hotel.name} className="h-16 w-16 rounded-md object-cover" />
        <div>
      ...
      <div className="mt-4 flex flex-col gap-1 border-t border-neutral-100 pt-4 text-sm">
        <div className="flex justify-between">
          ...
          <span className="ltr-content font-medium text-neutral-900">{checkIn || '—'}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-neutral-600">خروج</span>
          <span className="ltr-content font-medium text-neutral-900">{checkOut || '—'}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-neutral-600">مهمانان</span>
          <span className="font-medium text-neutral-900">{adults} بزرگسال، {children} کودک</span>
        </div>
        <div className="flex justify-between">
          <span className="text-neutral-600">تعداد شب</span>
          <span className="tabular-nums font-medium text-neutral-900">{nights}</span>
        </div>
      </div>
      ...
      <div className="mt-4 flex flex-col gap-1 border-t border-neutral-100 pt-4 text-sm">
        <div className="flex justify-between">
          ...
          <span className="tabular-price text-neutral-900">{formatToman(subtotal)}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-neutral-600">مالیات</span>
          <span className="tabular-price text-neutral-900">{formatToman(taxAmount)}</span>
        </div>
        {discount > 0 && (
